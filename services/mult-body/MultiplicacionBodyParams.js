...
const server = http.createServer(async (req, res) => {
  ...
  // === BODY: POST /multiplicar  (JSON) ===
  if (req.method === 'POST' && pathname === '/multiplicar') {
    try {
      let raw = '';
      ...
      req.on('end', () => {
        ...
        const body = { ok: true, op: 'MULT', num1: n1, num2: n2, result };
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify(body));

        // Log (no bloqueante)
        try {
          logOperation({ op: 'MULT', num1: n1, num2: n2, result, req, statusCode: 200, endpoint: '/multiplicar', source: 'ms' });
        } catch (_) {}
      });
      return;
    } catch (err) {
  ...
  if (req.method === 'GET' && pathname === '/multiplicar') {
    const msg = { ok: false, error: 'Usa POST /multiplicar con JSON: { "num1": 1, "num2": 2 }' };
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`MULTIPLICAR (body) escuchando en http://0.0.0.0:${PORT}/multiplicar`)
});