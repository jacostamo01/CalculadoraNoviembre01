...
const server = http.createServer(async (req, res) => {
  ...
  // === BODY: POST /dividir  (JSON) ===
  if (req.method === 'POST' && pathname === '/dividir') {
    try {
      let raw = '';
      ...
      req.on('end', () => {
        ...
        const body = { ok: true, op: 'dividir', num1: n1, num2: n2, result };
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify(body));

        // Log (no bloqueante)
        try {
          logOperation({ op: 'DIV', num1: n1, num2: n2, result, req, statusCode: 200, endpoint: '/dividir', source: 'ms' });
        } catch (_) {}
      });
      return;
    } catch (err) {
  ...
  if (req.method === 'GET' && pathname === '/dividir') {
    const msg = { ok: false, error: 'Usa POST /dividir con JSON: { "num1": 1, "num2": 2 }' };
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`DIVISION (Body) corriendo en http://0.0.0.0:${PORT}/dividir`);
});