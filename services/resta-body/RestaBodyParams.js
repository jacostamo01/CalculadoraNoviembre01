...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && pathname === '/restar') {
    try {
      let raw = '';
      ...
      req.on('end', () => {
        ...
        const body = { ok: true, op: 'RESTA', num1: n1, num2: n2, result };
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify(body));

        // Log (no bloqueante)
        try {
          logOperation({ op: 'RESTA', num1: n1, num2: n2, result, req, statusCode: 200, endpoint: '/restar', source: 'ms' });
        } catch (_) {}
      });
      return;
    } catch (err) {
  ...
  if (req.method === 'GET' && pathname === '/restar') {
    const msg = { ok: false, error: 'Usa POST /restar con JSON: { "num1": 1, "num2": 2 }' };
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Resta (Body)  escuchando en http://0.0.0.0:${PORT}/Restar`);
});