// RestaPathParams.js  (CommonJS)
'use strict';

const http = require('http');
const { logOperation } = require('./loggerClient'); // Archivo loggerClient.js
...
const server = http.createServer((req, res) => {
  ...
  if (parts[0] !== 'restar' || parts.length !== 3) {
    res.writeHead(404, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: false, error: 'Ruta inválida. Use /restar/{Num1}/{Num2}' }));
  }
  ...
  Promise.resolve(
    logOperation({ op: 'RESTA', num1: n1, num2: n2, result: resultado, req, statusCode: 200 })
  ).catch(() => {});
});

server.on('error', (err) => {
  // Si ves EADDRINUSE, el puerto está ocupado
  console.error('[RestaPathParams] server error:', err.code || err.message);
});

server.listen(PORT, '0.0.0.0', () => {
  console.log(`Resta (path) escuchando en http://0.0.0.0:${PORT}/restar`);
});
